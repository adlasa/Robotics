...
public class VisionThingy
{
    ...
    public VisionThingy(){
        ...
        SmartDashboard.putString("Vision Status", "Initialised");
        System.out.println("Vision Initialised");
    }
    ...
    {
        System.out.println("mainVision() run");
        VisionThingy.TargetReport target = new VisionThingy.TargetReport();
        ...
        try
        {
            ...
            VisionThingy.Scores scores[] = new VisionThingy.Scores[filteredImage.getNumberParticles()];
            ...
            if(filteredImage.getNumberParticles() > 0)
            {
                for(int i = 0; i < MAX_PARTICLES && i < filteredImage.getNumberParticles(); i++)
                {
                    ...
                    scores[i] = new VisionThingy.Scores();
                    ...
                    SmartDashboard.putDouble("rectangularity", scores[i].rectangularity);
        ...
        {
            ex.printStackTrace();
            SmartDashboard.putString("Vision Errors","NIVisionException");
        } catch(AxisCameraException ex)
        {
            ex.printStackTrace();
            SmartDashboard.putString("Vision Errors","AxisCameraException");
        }
        return;
    }
     ...
     */
    private boolean scoreCompare(VisionThingy.Scores scores, boolean vertical)
    {
     ...
     */
    private boolean hotOrNot(VisionThingy.TargetReport target)
    {
        boolean checkHot = true;

        checkHot &= target.tapeWidthScore >= TAPE_WIDTH_LIMIT;
        checkHot &= target.verticalScore >= VERTICAL_SCORE_LIMIT;
        checkHot &= (target.leftScore > LR_SCORE_LIMIT) | (target.rightScore > LR_SCORE_LIMIT);
